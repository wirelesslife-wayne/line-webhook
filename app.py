...
def get_auto_reply(text):
    ...
    if any(k in text for k in ['地址', '在哪', '門市', '位置', '怎麼去']):
        return '您好！門市地址請洽 LINE 客服確認，建議先預約再前往！'
    ...
    if any(k in text for k in ['預約', '約時間', '安排', '什麼時候可以']):
        return '您好！預約請提供：姓名、聯絡電話、車款、安裝項目、方便的日期時段，謝謝！'
    if any(k in text for k in ['保固', '保修', '壞掉', '故障']):
        return '您好！關於保固與維修，請提供您的車款與購買資訊，技術人員會盡快為您處理！'
    return None
...
def health():
    return 'LINE Webhook Server is running - WirelessLife', 200
